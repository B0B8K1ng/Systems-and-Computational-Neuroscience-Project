...
# Gengsheng Li added 2024-11-23 For running 288x288 mode
...
def main():
    ...
    if args.dataset in dvs_utils.DVS_DATASET:
        ...
        # Gengsheng Li 2024-11-23 added For debugging
        print("!!!!!! Using large-scale mode !!!!!!")
    ...
    try:
        ...
        if args.local_rank == 0:
            ...
            # Gengsheng Li changed 2024-11-23 For debugging
            # _logger.info("top-1:", eval_metrics["top1"])
            _logger.info(f"top-1: {eval_metrics['top1']:.4f}")
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                # Gengsheng Li added for Running the 288x288 mode
                if args.amp and not isinstance(input, torch.cuda.HalfTensor):
                    input = input.half()  # 如果使用AMP，转换为half类型'
                input = input.cuda()
